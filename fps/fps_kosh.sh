...
cat <<END
  ...
  SYNOPSIS
    /fps_kosh.sh [-x resolution] [-y resolution]
  
  DESCRIPTION
    macOS script for automating fps during kosh arrival

    Run from the root of your X-Plane 11 directory. Default resolution is 3840x2160.
    fisk_arrival.rep must be present in ./Output/replays
...
run() {
  ...
  do
    ...
    start=$SECONDS
    ./X-Plane.app/Contents/MacOS/X-Plane --fps_test=$i --load_smo=Output/replays/fisk_arrival.rep ---pref:_x_res_full_ALL=$xval --pref:_y_res_full_ALL=$yval 2>/dev/null
